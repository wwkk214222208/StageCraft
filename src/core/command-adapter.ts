...
/** 将稳定 Core Command 协议转发到现有 RoomRuntime facade。 */
export async function dispatchLegacyCommand(context: CoreCommandContext, command: HumanCommand): Promise<void> {
  ...
  switch (command.type) {
    case 'submit-text':
      ...
      return
    case 'select-role':
      await runtime.speak(roomId, String(payload.roleId ?? ''))
      return
    case 'cancel':
    ...
    case 'approve': {
      ...
      runtime.approve(roomId, String(payload.draftId ?? ''), String(payload.text ?? ''), (payload.stateUpdates ?? {}) as Record<string, string>, payload.sceneUpdates as { time?: string; location?: string } | undefined)
      return
    }
    case 'reject':
      if (payload.action === 'world-change') {
    ...
    case 'choose':
      if (typeof payload.roleId === 'string') {
        await runtime.speak(roomId, payload.roleId)
        return
      }
