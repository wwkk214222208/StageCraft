  ...
  return {
    ...
    async handle(command) {
      ...
      switch (operation) {
        ...
        case 'intervene-role': if (!nonEmptyString(payload.roleId) || typeof payload.selfModel !== 'string') throw new Error('Role intervention requires roleId and selfModel.'); return management.interveneRole(roomId, String(payload.roleId), payload.selfModel, Array.isArray(payload.memories) ? payload.memories as import('../types.ts').InitialMemory[] : undefined, payload.config && typeof payload.config === 'object' ? payload.config as Parameters<StageCraftManagementPort['interveneRole']>[4] : {})
