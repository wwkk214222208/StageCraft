          ...
          if (plugin.kind === 'llm-system') {
            try {
              ...
              const startContext: LlmSystemStartContext = {
                ...
                secrets: (() => { const requested = component.manifest.capabilities?.required ?? []; const optional = component.manifest.capabilities?.optional ?? []; const granted = context.host.capabilities?.includes('host.secrets') === true && (requested.includes('host.secrets') || optional.includes('host.secrets')); return granted ? { get: async key => { const value = (await context.host.call('host.secrets.get', { key }, caller) as { value?: unknown } | undefined)?.value; return typeof value === 'string' ? value : undefined }, set: async (key, value) => { await context.host.call('host.secrets.set', { key, value }, caller) }, delete: async key => { await context.host.call('host.secrets.delete', { key }, caller) }, has: async key => Boolean((await context.host.call('host.secrets.has', { key }, caller) as { found?: unknown } | undefined)?.found) } : undefined })(),
              }
