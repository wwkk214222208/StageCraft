...
test('M9 real example chain: build components, launch plan, Host, Solution, LLM stream/usage and Tool', async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/api/v2/core/invoke`, { method: 'POST', headers: { 'x-stagecraft-token': host.authToken }, body: JSON.stringify({ operation: 'demo/run', input: { user: 'hello', tool: 7 } }) }); const responseBody = await response.json() as any; assert.equal(response.status, 200, JSON.stringify(responseBody))
