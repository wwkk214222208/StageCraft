...
import { createAuthoringLlmSystemHarness, createDefaultLlmSystemService, defineLlmSystem, defineProviderDriver, defineSolution, STAGECRAFT_AUTHORING_API } from '../src/sdk/index.ts'
...
test('LLM System owns drivers, routing, credentials, stream and usage', async () => {
  ...
  const drivers = [make('one', 'one'), make('two', 'two')]
  const system = defineLlmSystem({ id: 'example.llm', version: '0.1.0', title: 'LLM', async start(c) { const service = await createDefaultLlmSystemService(c); await service.upsertCredentialProfile({ id: 'p1', providerId: 'two', label: 'Two' }); return service } })
  const h = await createAuthoringLlmSystemHarness(system, {}, { drivers })
  assert.deepEqual(h.listModels(), [{ providerId: 'one', models: ['one-model'] }, { providerId: 'two', models: ['two-model'] }])
  assert.deepEqual(await h.route({ providerId: 'two', model: 'two-model', credentialProfileId: 'p1' }), { providerId: 'two', model: 'two-model', credentialProfileId: 'p1' })
  const chunks = []; for await (const c of h.complete({ requestId: 'r1', providerId: 'two', model: 'two-model', credentialProfileId: 'p1', messages: [{ role: 'user', content: 'assembled' }], credential: { profileId: 'p1', secret: 'secret-value' } })) chunks.push(c.type)
...
test('cancellation targets exactly one request and preserves Solution messages', async () => {
  ...
  const system = defineLlmSystem({ id: 'example.cancel-system', version: '0.1.0', title: 'Cancel system', start: c => createDefaultLlmSystemService(c) })
  const h = await createAuthoringLlmSystemHarness(system, {}, { drivers: [driver] }); const iterator = h.complete({ requestId: 'target', messages: [{ role: 'system', content: 'owned by solution' }, { role: 'user', content: 'hi' }] })[Symbol.asyncIterator]()
...
test('active duplicate requestId is rejected instead of replacing the cancellable request', async () => {
  ...
  const h = await createAuthoringLlmSystemHarness(defineLlmSystem({ id: 'example.duplicate-system', version: '0.1.0', title: 'Duplicate', start: c => createDefaultLlmSystemService(c) }), {}, { drivers: [driver] })
...
test('metadata is immutable and route rejects unknown driver/model/profile', async () => {
  ...
  const h = await createAuthoringLlmSystemHarness(defineLlmSystem({ id: 'example.s', version: '0.1.0', title: 'S', start: c => createDefaultLlmSystemService(c) }), {}, { drivers: [driver] })
  await assert.rejects(h.route({ providerId: 'missing', model: 'm' }), /no provider driver/)
...
test('Solution system message is passed through without LLM mutation', async () => {
  ...
  const h = await createAuthoringLlmSystemHarness(defineLlmSystem({ id: 'example.pass-system', version: '0.1.0', title: 'Pass', start: c => createDefaultLlmSystemService(c) }), {}, { drivers: [driver] }); for await (const _ of h.complete({ requestId: 'x', messages })) {}
