...
test('StageCraft chat commands use the solution handler instead of the legacy dispatcher', async () => {
  ...
  await core.dispatch({ id: 'non-chat-choose', actor: 'player', type: 'choose', payload: { roleId: 'aria', scope: 'director' } })
  await core.dispatch({ id: 'non-chat-cancel', actor: 'player', type: 'cancel', payload: { scope: 'director' } })
  await core.dispatch({ id: 'non-chat-retry', actor: 'player', type: 'retry', payload: { scope: 'director' } })
