...
test('sync roles to story: 运行中角色卡写回初始剧本，重开即用新角色', async () => {
  ...
  const story = {
    ...
    roles: [{ id: 'aria', name: 'Aria', portraitRef: '/a.svg', currentState: '在场。', presence: 'present', memoryTimeline: { '未标注时间': ['原始记忆。'] }, selfModel: '原始人设。', impressions: { 旅人: '剧本里已有的印象。' } }],
  }
  ...
  // 运行中：修改人设 + 更新印象 + 新建角色
  runtime.interveneRole(roomId, 'aria', '修改后的人设。', { '未标注时间': ['修改后记忆。'] }, { impressions: { 旅人: '运行中更新的印象。' } })
  runtime.createRole(roomId, { id: 'newbie', name: '新人', portraitRef: '/n.svg', currentState: '站在门口。', presence: 'present', selfModel: '新人设。', memoryTimeline: { '未标注时间': ['刚出场。'] } })
...
test('sync role to story: 单角色同步保留印象；room 印象为空时保留剧本原印象', () => {
  ...
  runtime.interveneRole(roomId, 'aria', '修改后的人设。', { '未标注时间': ['修改后记忆。'] })
  const room = runtime.get(roomId)
