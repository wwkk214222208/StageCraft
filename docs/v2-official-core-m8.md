...
LLM System 现在是完整可替换的 `LlmSystemService`：插件 `start(context)` 接收选中的独立 `provider-driver`、配置/日志和可选 state/secret ports，并返回 service。Driver 只处理 LLM System 已决定的 provider/model/credential/messages，不获得全局路由或凭据存储权；Core/runtime 不创建管理 harness。Service 自身拥有 status、driver/model catalog、credential profile/secret、route、流式 complete、request-scoped cancel 和 usage record/query/aggregate/stop。当前 official runtime 只提供已存在的 state port；secret port 尚未接入，因此参考实现启用官方 runtime 时会将 secret 与其组件 state 一起持久化，属于开发/参考限制。Solution 的 `systemPrompt` 原样成为 system message，LLM 接收的 messages 由调用方/系统传入，不由 LLM 或 Driver 拼接。`createDefaultLlmSystemService` 仅是插件可显式选择的参考实现，不能由宿主替插件实现管理。
