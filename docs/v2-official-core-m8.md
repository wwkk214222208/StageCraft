...
LLM System 现在是完整可替换的 `LlmSystemService`：插件 `start(context)` 接收选中的独立 `provider-driver`、配置/日志和可选 state/secret ports，并返回 service。Driver 只处理 LLM System 已决定的 provider/model/credential/messages，不获得全局路由或凭据存储权；Core/runtime 不创建管理 harness。Service 自身拥有 status、driver/model catalog、credential profile/secret、route、流式 complete、request-scoped cancel 和 usage record/query/aggregate/stop。Host secret port 是可选能力：桌面参考 Host 默认仍只提供 `host.log`/`host.storage`，Android Host 提供按组件命名空间隔离的 Keystore-backed `host.secrets`（`get/set/delete/has`）；未协商该能力的插件必须自行处理无 secret port 的限制，不得把秘密伪装成普通 state。Solution 的 `systemPrompt` 原样成为 system message，LLM 接收的 messages 由调用方/系统传入，不由 LLM 或 Driver 拼接。`createDefaultLlmSystemService` 仅是插件可显式选择的参考实现，不能由宿主替插件实现管理。
