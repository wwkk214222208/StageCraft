...
public final class AndroidCompositionOperations implements AndroidNativeOperations, AutoCloseable {
    ...
    @Override public Object invokeSync(String operation, JSONObject input) throws Exception {
        ...
        if ("stagecraft.room.get".equals(operation)) {
            ...
            // 对外只暴露 active 记忆（软删/被替代的记忆保留在内部但不可见，与桌面 listNpcMemories 默认过滤一致）
            org.json.JSONArray roles = room.optJSONArray("roles");
            if (roles != null) {
                for (int i = 0; i < roles.length(); i++) {
                    JSONObject role = roles.optJSONObject(i);
                    if (role == null) continue;
                    org.json.JSONArray all = role.optJSONArray("memories");
                    if (all == null) continue;
                    org.json.JSONArray active = new org.json.JSONArray();
                    for (int j = 0; j < all.length(); j++) {
                        JSONObject memory = all.optJSONObject(j);
                        if (memory == null) continue;
                        String status = memory.optString("status", "active");
                        if ("active".equals(status)) active.put(memory);
                    }
                    role.put("memories", active);
                }
            }
            return room;
        }
        ...
        executor.execute(() -> {
            try {
                ...
                if ("model.request".equals(operation)) {
                    ...
                    String requestId = JsonSafety.requiredString(input, "requestId", 256);
                    JSONObject providerBody = input.optJSONObject("body");
                    if (providerBody == null) throw new IllegalArgumentException("model request body is missing");
                    modelTransport.request(endpoint, apiKey, requestId, providerBody, new AndroidModelTransport.Listener() {
    ...
    private Object applyRoleOrMemory(JSONObject room, String method, org.json.JSONArray args) throws Exception {
        ...
        if (method.contains("Memory")) { JSONObject role = find(roles, "id", JsonSafety.stringArg(args, 1, 256)); if (role == null) throw new IllegalArgumentException("Role not found."); org.json.JSONArray memories = array(role, "memories"); String memoryId = args.length() > 2 && !args.isNull(2) ? args.optString(2, "") : ""; if ("insertNpcMemories".equals(method)) { org.json.JSONArray incoming = JsonSafety.arrayArg(args, 2); for (int i = 0; i < incoming.length(); i++) { JSONObject m = incoming.optJSONObject(i); if (m != null && !m.has("status")) m.put("status", "active"); } role.put("memories", incoming); } else if ("reorderNpcMemories".equals(method)) { org.json.JSONArray ordered = JsonSafety.arrayArg(args, 2); if (ordered.length() != memories.length()) throw new IllegalArgumentException("记忆顺序列表与现有记忆不一致。"); java.util.HashSet<String> seen = new java.util.HashSet<>(); for (int i = 0; i < ordered.length(); i++) { String id = ordered.optString(i, ""); if (id.isEmpty() || !seen.add(id) || find(memories, "id", id) == null) throw new IllegalArgumentException("记忆顺序列表与现有记忆不一致。"); } role.put("memories", ordered); } else if ("retractNpcMemory".equals(method)) { JSONObject found = find(memories, "id", memoryId); if (found == null) throw new IllegalArgumentException("记忆不存在。"); found.put("status", "retracted"); } else if ("supersedeNpcMemory".equals(method)) { JSONObject replacement = JsonSafety.objectArg(args, 2); if (memoryId.isEmpty()) memoryId = replacement.optString("id", ""); JSONObject found = find(memories, "id", memoryId); if (found == null) throw new IllegalArgumentException("可替代的记忆不存在。"); if (!replacement.has("status")) replacement.put("status", "active"); memories.put(new JSONObject(replacement.toString()).put("id", replacement.optString("id", memoryId))); found.put("status", "superseded").put("supersededBy", replacement.optString("id", memoryId)); } else if ("updateNpcMemory".equals(method)) { JSONObject found = find(memories, "id", memoryId); if (found == null) throw new IllegalArgumentException("记忆不存在。"); JSONObject patch = JsonSafety.objectArg(args, 2); for (java.util.Iterator<String> keys = patch.keys(); keys.hasNext(); ) { String key = keys.next(); found.put(key, patch.get(key)); } } bump(room); return JSONObject.NULL; }
