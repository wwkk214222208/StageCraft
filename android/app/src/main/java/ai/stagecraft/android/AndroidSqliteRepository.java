...
public final class AndroidSqliteRepository extends SQLiteOpenHelper {
    ...
    public synchronized void saveCoreState(String roomId, long revision, JSONObject state, JSONObject events, JSONObject workflows) { SQLiteDatabase db = getWritableDatabase(); db.beginTransaction(); try { ContentValues row = new ContentValues(); row.put("room_id", roomId); row.put("revision", revision); row.put("state", state.toString()); row.put("events", events.toString()); row.put("workflows", workflows.toString()); row.put("updated_at", System.currentTimeMillis()); db.insertWithOnConflict("core_state", null, row, SQLiteDatabase.CONFLICT_REPLACE); db.setTransactionSuccessful(); } finally { db.endTransaction(); } }
    public synchronized JSONObject loadCoreState(String roomId) throws Exception { try (Cursor cursor = getReadableDatabase().query("core_state", new String[]{"revision","state","events","workflows"}, "room_id=?", new String[]{roomId}, null, null, null)) { if (!cursor.moveToFirst()) return null; return new JSONObject().put("revision", cursor.getLong(0)).put("state", new JSONObject(cursor.getString(1))).put("events", new JSONObject(cursor.getString(2))).put("workflows", new JSONObject(cursor.getString(3))); } }
