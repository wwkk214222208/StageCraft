...
function templateSource(kind, id, title) {
  ...
  if (kind === 'llm-system') return `${header}const plugin = defineLlmSystem({ id: '${id}', version: '0.1.0', title: ${titleLiteral}, start: context => createDefaultLlmSystemService(context) })\nexport default plugin\n`
...
function templateTest(kind, id) {
  ...
  if (kind === 'llm-system') return `${header}test('llm system returns a complete service', async () => { const { createAuthoringLlmSystemHarness } = await import('../src/stagecraft-sdk.ts'); const service = await createAuthoringLlmSystemHarness(plugin); assert.equal(service.status, 'ready'); for (const name of ['route', 'complete', 'cancel', 'queryUsage', 'aggregateUsage', 'stop']) assert.equal(typeof service[name], 'function') })\n`
