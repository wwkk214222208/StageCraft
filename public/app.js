...
function renderStructuredMemories(role) { const list = $('#inspector-memory-structured'); if (!list) return; const memories = role?.memories ?? []; list.innerHTML = memories.length ? memories.map(memory => `<article class="memory-record" data-memory-id="${escape(memory.id)}"><header><b>${escape(memory.kind)}</b><small>${escape(memory.occurredAt)} · 重要度 ${memory.salience}</small></header><p>${escape(memory.text)}</p><button type="button" data-memory-retract="${escape(memory.id)}">撤回</button></article>`).join('') : '<p class="hint">暂无结构化记忆</p>' }
document.addEventListener('click', event => { const memoryId = event.target.closest('[data-memory-retract]')?.dataset.memoryRetract; if (memoryId) api('/api/roles/memories/retract', { memoryId }).then(refreshRoom) })
