...
const promptPresetScopes = { 'director.role-decision': '导演模式 · 角色决策', 'director.draft': '导演模式 · 场景草稿', 'director.consult': '导演模式 · 导演咨询', 'director.memory-digest': '导演模式 · 记忆消化', 'chat.role-speech': '群聊模式 · 角色发言', 'chat.world-director': '群聊模式 · 世界导演', 'chat.role-selection': '群聊模式 · 导演选角', 'prompt-preset.transform': 'AI 预设助手' }
...
function render(next) {
  ...
  else if (room.phase === 'awaiting-player-input') progressText = `<p>${isChat ? `群聊模式：写下你的行动，再${room.speechMode === 'director' ? '让导演安排发言' : room.speechMode === 'all' ? '所有人依次发言' : '点左侧角色的「发言」'}。` : '等待你的行动。'}</p>`
...
// 群聊发言模式：侧栏「设置与预设」切换（持久化）与模式触发按钮
...
document.addEventListener('click', event => { const trigger = event.target.closest?.('#chat-mode-trigger'); if (trigger) { if (activeAction) return; activeAction = 'speak'; render(room); api(room.speechMode === 'director' ? '/api/chat/director-decide' : '/api/chat/speak-all', {}).finally(() => { activeAction = null; refreshRoom() }) } })
...
document.addEventListener('change', async event => { const sideSelect = event.target.closest?.('#prompt-preset-select-sidebar'); if (sideSelect) { sidebarPromptPresetId = sideSelect.value || 'default'; if (sidebarPromptPresetId !== 'default') { void fetch('/api/prompts/presets', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ scope: promptPresetScope, activePresetId: sidebarPromptPresetId }) }).then(response => { if (!response.ok) alert('切换当前预设失败。') }).catch(error => alert(error.message)) } render(room); return } const privateToggle = event.target.closest?.('.prompt-private-toggle'); if (!privateToggle) return; try { const response = await fetch('/api/prompts/private-toggles', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ presetId: sidebarPromptPresetId, nodeId: privateToggle.dataset.promptPrivate, enabled: privateToggle.checked }) }); if (!response.ok) throw new Error('save failed') } catch (error) { alert('开关状态保存失败。'); } await loadPromptPresets(); render(room) })
...
$('#prompt-preset-save').onclick = async () => { if (editingPromptPreset?.id === 'default') { alert('默认方案不可覆盖保存，请用另存为创建新预设。'); return }; const preset = collectPromptPreset(); const response = await fetch('/api/prompts/presets', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ preset }) }); if (!response.ok) { alert('保存提示词预设失败。'); return }; const data = await response.json(); promptPresets = data.presets ?? promptPresets; if ($('#prompt-preset-enabled').checked) { const active = await fetch('/api/prompts/presets', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ scope: promptPresetScope, activePresetId: preset.id }) }); if (!active.ok) { alert('作用域预设设置失败。'); return } } $('#prompt-preset-modal').close(); render(room) }
