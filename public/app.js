...
function render(next) {
  ...
  const promptSideSelect = $('#prompt-preset-select-sidebar'); if (promptSideSelect) { const sideOptions = [...(promptPresets.some(item => item.id === 'default') ? [] : [DEFAULT_PROMPT_PRESET()]), ...promptPresets]; promptSideSelect.innerHTML = sideOptions.map(item => `<option value="${escape(item.id)}">${escape(item.id === 'default' ? '默认预设' : item.name)}</option>`).join(''); promptSideSelect.value = sideOptions.some(item => item.id === sidebarPromptPresetId) ? sidebarPromptPresetId : 'default' }
...
document.addEventListener('change', async event => { const sideSelect = event.target.closest?.('#prompt-preset-select-sidebar'); if (sideSelect) { sidebarPromptPresetId = sideSelect.value || 'default'; render(room); return } const privateToggle = event.target.closest?.('.prompt-private-toggle'); if (!privateToggle) return; try { const response = await fetch('/api/prompts/private-toggles', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ presetId: sidebarPromptPresetId, nodeId: privateToggle.dataset.promptPrivate, enabled: privateToggle.checked }) }); if (!response.ok) throw new Error('save failed') } catch (error) { alert('开关状态保存失败。'); } await loadPromptPresets(); render(room) })
