...
function renderStructuredMemories(role) { const list = $('#inspector-memory-structured'); if (!list) return; const memories = role?.memories ?? []; list.innerHTML = `<button type="button" class="small-btn" data-memory-add="${escape(role.id)}">＋ 新增记忆</button>${memories.length ? `<div class="memory-list-rows">${memories.map(memory => `<article class="memory-list-row" data-memory-id="${escape(memory.id)}"><input data-memory-time value="${escape(memory.occurredAt ?? '未标注时间')}" aria-label="记忆时间"><textarea data-memory-inline rows="1" aria-label="记忆内容">${escape(memory.text)}</textarea><button type="button" class="memory-delete" data-memory-retract="${escape(memory.id)}" title="删除记忆">×</button></article>`).join('')}</div>` : '<p class="hint">暂无结构化记忆</p>'}` }
...
document.addEventListener('click', event => { const target = event.target.closest('[data-memory-retract],[data-memory-add]'); if (!target) return; const roleId = target.dataset.memoryAdd; const memoryId = target.dataset.memoryRetract; const role = room?.roles.find(item => item.id === (roleId || inspectedRole?.id)); const current = role?.memories?.find(item => item.id === memoryId); if (!role) return; if (target.dataset.memoryAdd) return openMemoryEditor(role); if (target.dataset.memoryRetract) return api('/api/roles/memories/retract', { memoryId }).then(refreshRoom); if (current) openMemoryEditor(role, current) })
document.addEventListener('focusin', event => { if (event.target.matches?.('[data-memory-inline]')) event.target.classList.add('expanded') })
document.addEventListener('focusout', event => { if (!event.target.matches?.('[data-memory-inline]')) return; event.target.classList.remove('expanded'); const row = event.target.closest('[data-memory-id]'); if (!row) return; api('/api/roles/memories/update', { memoryId: row.dataset.memoryId, entry: { text: event.target.value.trim(), occurredAt: row.querySelector('[data-memory-time]').value.trim() || '未标注时间' } }).then(refreshRoom) })
