...
function tokenNoteHtml(kind, usage) {
  ...
  return `<small class="token-note" title="本段由模型生成时的 token 用量">${labels[kind] ?? kind} ↑${usage.promptTokens ?? 0}+${usage.completionTokens ?? 0} tokens</small>`
}
...
function render(next) {
  ...
  const roleCards = room.roles.map(role => {
    ...
    const speakBtn = isChat && role.presence === 'present' && room.phase === 'awaiting-player-input' ? `<button class="speak-toggle" data-speak="${escape(role.id)}">发言</button>` : ''
  ...
  let consultHtml = (room.consultations ?? []).map(message => `<p class="consultation ${message.role}"><b>${message.role === 'player' ? room.playerCharacter.name : '导演'}</b>${escape(message.text)}${tokenNoteHtml('consult', message.usage)}</p>`).join('')
  ...
  $('#director-chat').hidden = isChat
  const consultAvailable = room.draft && !['awaiting-player-input', 'drafting'].includes(room.phase)
  $('#consult-send').disabled = isChat ? true : activeAction === 'director' ? false : !consultAvailable
  $('#consult-send').textContent = isChat ? '群聊模式禁用' : activeAction === 'director' ? '停止' : room.draft ? '发送' : room.phase === 'awaiting-player-input' ? '设定' : '发送'
  $('#consult-text').disabled = isChat
  $('#consult-text').placeholder = isChat ? '群聊模式：导演区不可用，直接点角色发言。' : room.draft ? '与导演讨论、指出问题，或要求调整当前草稿...' : '向导演说明你的设定，将作为后续起草参考...'
...
thinkingEvents.addEventListener('thinking', event => applyThinkingEvent(JSON.parse(event.data)))
