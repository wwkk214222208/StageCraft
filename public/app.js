...
function render(next) {
  ...
  const defaultPresetCard = promptPresets.some(item => item.id === 'default') ? '' : '<article class="prompt-preset-row"><button type="button" class="prompt-preset-name" data-prompt-preset="default">默认预设</button><span>默认</span><button type="button" data-prompt-copy="default">复制</button></article>'
  const presetCards = defaultPresetCard + promptPresets.map(preset => `<article class="prompt-preset-row"><button type="button" class="prompt-preset-name" data-prompt-preset="${escape(preset.id)}">${escape(preset.name)}</button><span>${Object.values(promptPresetState.activeByScope ?? {}).includes(preset.id) || preset.enabled ? '使用中' : '未使用'}</span><button type="button" data-prompt-copy="${escape(preset.id)}">复制</button>${preset.id === 'default' ? '' : '<button type="button" data-prompt-delete="${escape(preset.id)}">删除</button>'}</article>`).join('')
