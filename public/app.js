...
// 侧栏私设条目开关：状态独立保存（localStorage），预设文件里的 enabled 仅作为加载时的初始开关状态。
const promptToggleStoreKey = 'stagecraft.prompt-private-toggles'
let promptToggleOverrides = {}
function loadPromptToggleOverrides() { try { promptToggleOverrides = JSON.parse(localStorage.getItem(promptToggleStoreKey) ?? '{}') } catch { promptToggleOverrides = {} } return promptToggleOverrides }
function savePromptToggleOverride(presetId, nodeId, enabled) { const store = loadPromptToggleOverrides(); store[presetId] = store[presetId] ?? {}; store[presetId][nodeId] = enabled; localStorage.setItem(promptToggleStoreKey, JSON.stringify(store)) }
function promptNodeEnabled(presetId, node) { const over = loadPromptToggleOverrides()[presetId]?.[node.id]; return over === undefined ? node.enabled !== false : over }
loadPromptToggleOverrides()
...
$('#prompt-preset-select-sidebar').onchange = () => { sidebarPromptPresetId = $('#prompt-preset-select-sidebar')?.value || 'default'; render(room) }
document.addEventListener('change', event => { const privateToggle = event.target.closest?.('.prompt-private-toggle'); if (!privateToggle) return; savePromptToggleOverride(sidebarPromptPresetId, privateToggle.dataset.promptPrivate, privateToggle.checked); render(room) })
