...
document.addEventListener('change', async event => { const privateToggle = event.target.closest?.('.prompt-private-toggle'); if (!privateToggle) return; try { const response = await fetch('/api/prompts/private-toggles', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ presetId: sidebarPromptPresetId, nodeId: privateToggle.dataset.promptPrivate, enabled: privateToggle.checked }) }); if (!response.ok) throw new Error('save failed') } catch (error) { alert('开关状态保存失败。'); } await loadPromptPresets(); render(room) })
