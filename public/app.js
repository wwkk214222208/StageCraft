...
function collectStoryInitialMemories() { return [...document.querySelectorAll('#story-initial-memories .memory-record')].map((_, index) => { const field = name => document.querySelector(`#story-initial-memories [data-story-memory="${name}"][data-index="${index}"]`); return { kind: field('kind').value, text: field('text').value.trim(), subjects: field('subjects').value.split(/[,，、]/).map(item => item.trim()).filter(Boolean), occurredAt: field('occurredAt').value.trim() || '未标注时间', salience: Math.max(1, Math.min(5, Number(field('salience').value) || 3)), confidence: 1 } }).filter(memory => memory.text) }
document.addEventListener('click', event => { const add = event.target.closest?.('#story-memory-add'); const remove = event.target.closest?.('[data-story-memory-delete]'); if (!add && !remove) return; const role = storyEditRoles[storyEditRoleIndex]; if (!role) return; role.initialMemories = collectStoryInitialMemories(); if (add) role.initialMemories.push({ kind: 'fact', text: '', subjects: [], occurredAt: '未标注时间', salience: 3, confidence: 1 }); else role.initialMemories.splice(Number(remove.dataset.storyMemoryDelete), 1); renderStoryInitialMemories(role) })
...
function renderStructuredMemories(role) { const list = $('#inspector-memory-structured'); if (!list) return; const memories = role?.memories ?? []; list.innerHTML = `<button type="button" class="small-btn" data-memory-add="${escape(role.id)}">＋ 新增记忆</button>${memories.length ? `<div class="memory-list-rows">${memories.map(memory => `<article class="memory-list-row" data-memory-id="${escape(memory.id)}"><div><b>${escape(memory.kind)}</b><small>${escape(memory.occurredAt)} · 重要度 ${memory.salience}</small></div><p title="${escape(memory.text)}">${escape(memory.text)}</p><small>${escape((memory.subjects ?? []).join('、') || '无关联人物')}</small><div><button type="button" data-memory-edit="${escape(memory.id)}">编辑</button><button type="button" data-memory-supersede="${escape(memory.id)}">替代</button><button type="button" data-memory-retract="${escape(memory.id)}">撤回</button></div></article>`).join('')}</div>` : '<p class="hint">暂无结构化记忆</p>'}` }
function openMemoryEditor(role, memory, mode = 'update') {
  ...
  const current = memory ?? { kind: 'fact', text: '', subjects: [], occurredAt: room?.sceneTime ?? '未标注时间', salience: 3, confidence: 1 }
  dialog.innerHTML = `<form method="dialog"><header><h3>${mode === 'supersede' ? '替代记忆' : memory ? '编辑记忆' : '新增记忆'}</h3><button type="button" data-memory-popover-close>关闭</button></header><label>类型<select id="memory-popover-kind">${['fact','observation','interaction','promise','relationship','belief','emotion','goal_update'].map(kind => `<option value="${kind}" ${current.kind === kind ? 'selected' : ''}>${kind}</option>`).join('')}</select></label><label>时间<input id="memory-popover-time" value="${escape(current.occurredAt ?? '未标注时间')}"></label><label>重要度<input id="memory-popover-salience" type="number" min="1" max="5" value="${current.salience ?? 3}"></label><label>关联人物（逗号分隔）<input id="memory-popover-subjects" value="${escape((current.subjects ?? []).join(', '))}"></label><label>记忆内容<textarea id="memory-popover-text" rows="8">${escape(current.text ?? '')}</textarea></label><footer><button type="button" data-memory-popover-close>取消</button><button type="submit">保存</button></footer></form>`
  dialog.querySelectorAll('[data-memory-popover-close]').forEach(button => { button.onclick = () => dialog.close() })
  dialog.querySelector('form').onsubmit = event => { event.preventDefault(); const entry = { kind: $('#memory-popover-kind').value, text: $('#memory-popover-text').value.trim(), subjects: $('#memory-popover-subjects').value.split(/[,，、]/).map(item => item.trim()).filter(Boolean), occurredAt: $('#memory-popover-time').value.trim() || '未标注时间', salience: Math.max(1, Math.min(5, Number($('#memory-popover-salience').value) || 3)), confidence: current.confidence ?? 1 }; if (!entry.text) return alert('记忆内容不能为空。'); const endpoint = mode === 'supersede' ? '/api/roles/memories/supersede' : memory ? '/api/roles/memories/update' : '/api/roles/memories'; const payload = memory ? { memoryId: memory.id, entry } : { roleId: role.id, entries: [entry] }; api(endpoint, payload).then(ok => { if (ok) { dialog.close(); refreshRoom() } }) }
  dialog.showModal()
}
document.addEventListener('click', event => { const target = event.target.closest('[data-memory-retract],[data-memory-add],[data-memory-edit],[data-memory-supersede]'); if (!target) return; const roleId = target.dataset.memoryAdd; const memoryId = target.dataset.memoryRetract || target.dataset.memoryEdit || target.dataset.memorySupersede; const role = room?.roles.find(item => item.id === (roleId || inspectedRole?.id)); const current = role?.memories?.find(item => item.id === memoryId); if (!role) return; if (target.dataset.memoryAdd) return openMemoryEditor(role); if (target.dataset.memoryRetract) return api('/api/roles/memories/retract', { memoryId }).then(refreshRoom); if (current) openMemoryEditor(role, current, target.dataset.memorySupersede ? 'supersede' : 'update') })
...
$('#inspector-save').onclick = event => {
  ...
  if (storyEditRoleIndex !== null) {
    ...
    if (role) {
      ...
      role.initialMemories = collectStoryInitialMemories()
      delete role.memoryTimeline
