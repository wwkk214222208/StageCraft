...
function promptSideScope() {
  ...
  return ['chat.role-speech', 'chat.world-director', 'chat.role-selection'].find(scope => promptPresetState?.activeByScope?.[scope]) ?? 'chat.role-speech'
}
...
function renderPromptPresetEditor() {
  ...
  if (editorTitle) editorTitle.textContent = `提示词编排 · 《${preset.name}》 · ${promptPresetScopes[promptPresetScope] ?? promptPresetScope}`
...
$('#prompt-preset-save').onclick = async () => {
  ...
  try {
    ...
    if ($('#prompt-preset-enabled').checked) {
      const active = await fetch('/api/prompts/presets', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ scope: promptPresetScope, activePresetId: preset.id }) })
      if (!active.ok) { alert('作用域预设设置失败。'); return }
    }
